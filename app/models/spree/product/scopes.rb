module Spree
  class Product < ActiveRecord::Base
    ...
    def self.available(available_on = nil, sell_finish_at = nil, currency = nil)
      joins(:master => :prices).where("#{Product.quoted_table_name}.available_on <= ? AND
                                         #{Product.quoted_table_name}.sell_finish_at >= ? AND
                                          #{Product.quoted_table_name}.sell_stop =?",
                                            available_on || Time.now, sell_finish_at|| Time.now, false)
    end
    search_scopes << :available

    def self.finished( sell_finish_at = nil, currency = nil)
      joins(:master => :prices).where("#{Product.quoted_table_name}.sell_finish_at <= ?", sell_finish_at|| Time.now)
    end
    search_scopes << :finished

    def self.stopped()
      joins(:master => :prices).where("#{Product.quoted_table_name}.sell_stop = ?", true)
    end
    search_scopes << :stopped

    def self.active(currency = nil)
      not_deleted.available(nil, nil, currency)
    end
